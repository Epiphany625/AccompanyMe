...
export const UserAvailabilityList = () => {
  ...
  const [editForm, setEditForm] =
    useState<AvailabilityFormState>(EMPTY_AVAILABILITY)
    ...
    const loadAvailabilities = async () => {
      ...
      try {
        ...
        setErrorMessage(
          typeof error === "string"
            ? error
            : "Unable to load availabilities. Please try again.",
        )
      } finally {
        if (isActive) {
          setIsLoading(false)
  ...
  const handleDelete = async (availabilityId: number) => {
    ...
    try {
      ...
      setErrorMessage(
        typeof error === "string" ? error : "Unable to delete availability.",
      )
    } finally {
  ...
  const handleSave = async (availabilityId: number) => {
    ...
    try {
      ...
      setErrorMessage(
        typeof error === "string" ? error : "Unable to update availability.",
      )
    } finally {
      setIsSaving(false)
    }
  ...
  const content = () => {
    ...
    if (errorMessage) {
      return (
        <p className="availability-status availability-status--error">
          {errorMessage}
        </p>
      )
    }
              ...
              {isEditing ? (
                <div className="availability-edit">
                  <div className="availability-edit-fields">
                    <div className="ds-field">
                      <label
                        className="ds-label"
                        htmlFor={`edit-start-${availability.id}`}
                      >
                        Start time
                      </label>
                      <input
                        ...
                        onChange={event =>
                          handleEditChange("startTime", event.target.value)
                        }
                      />
                    </div>
                    <div className="ds-field">
                      <label
                        className="ds-label"
                        htmlFor={`edit-duration-${availability.id}`}
                      >
                        Duration (minutes)
                      </label>
                      <input
                ...
                <>
                  ...
                  <div className="availability-item-actions">
                    ...
                    >
                      {deletingIds.includes(availability.id)
                        ? "Deleting..."
                        : "Delete"}
                    </Button>
                  </div>
      ...
      <header className="availability-panel-header">
        <div>
          ...
          <p className="availability-card-desc">
            These slots are currently open to book.
          </p>
        </div>
      </header>
      {content()}
